...
const BlogClient = ({}) => {
  ...
  const openEditPopup = (blog) => {
    ...
    SetPrice(blog.price || "");
    setCategory(blog.category || ""); // <-- add this
    setShowPopup(true);
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {loading ? (
              ...
              filteredBlogs.map((blog) => {
                const avgRating =
                  blog.reviews && blog.reviews.length
                    ? blog.reviews.reduce(
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            ...
            >
              ...
              >
                ...
                {updateLoading ? "Processing..." : "Update Product"}
              </button>
            </form>
          </div>
        )}
